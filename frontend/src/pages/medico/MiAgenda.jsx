...
function MiAgenda() {
  ...
  const handleDayClick = (day, current) => {
    if (!current || !doctorId) return;
    const fecha = `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
    setSelectedDate(fecha);
    setLoading(true);
    api.getCitasByMedico(doctorId, { fecha })
        ...
        {selectedDate && (
          <div className="table-card" style={{ marginTop: 20 }}>
            ...
            {loading ? <p style={{ color: '#888', padding: '12px 0' }}>Cargando...</p> : citas.length === 0 ? (
              ...
              <table>
                <thead>
                  <tr><th>Hora</th><th>Paciente</th><th>Motivo</th><th>Estado</th><th>Acciones</th></tr>
                </thead>
                <tbody>
                  {citas.map(c => (
                    <tr key={c.id}>
                      ...
                      <td>
                        {c.status === 'confirmada' && (
                          <button className="btn-action" onClick={() => navigate(`/medico/consulta/${c.id}`)}>
                            Iniciar consulta
                          </button>
                        )}
                      </td>
